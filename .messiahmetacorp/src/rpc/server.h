...
/**
 * MessiahMetaCorp RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZmmcSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzmmcseed(const UniValue& params, bool fHelp);
extern UniValue getzmmcseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzmmc(const UniValue& params, bool fHelp);
extern UniValue dzmmcstate(const UniValue& params, bool fHelp);
