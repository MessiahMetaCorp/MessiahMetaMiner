...
// The zMMC block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zMMC is a hash of the serial
...
{
    //Create an output returning the zMMC that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZMMCOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zMMC output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zMMC", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZMMCOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zMMC output", __func__);
...
{
    CzMMCTracker* zmmcTracker = pwallet->zmmcTracker.get();
    CMintMeta meta;
    if (!zmmcTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zmmcTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
