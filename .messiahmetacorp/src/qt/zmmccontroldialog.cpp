...
#include "ui_zmmccontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zmmcTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zmmcTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zMMC.";
            else if (!mint.isSeedCorrect)
                strReason = "The zMMC seed used to mint this zMMC is not the same as currently hold in the wallet";
            else
