...
CzMMCWallet::CzMMCWallet(std::string strWalletFile)
{
...
bool CzMMCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZMMCCount(nCountLastUsed);
    else if (!walletdb.ReadZMMCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzMMCWallet::Lock()
{
    seedMaster = 0;
}

void CzMMCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzMMCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZMMC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzMMCWallet::LoadMintPoolFromDB()
{
...
void CzMMCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzMMCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzMMCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zmmcTracker->HasPubcoinHash(pMint.first)) {
...
bool CzMMCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZMMC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    pwalletMain->zmmcTracker->Add(dMint, true);
...
void CzMMCWallet::SeedToZMMC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzMMCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzMMCWallet::GenerateDeterministicZMMC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzMMCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzMMCWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzMMCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
