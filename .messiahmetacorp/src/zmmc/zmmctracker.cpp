...
CzMMCTracker::CzMMCTracker(std::string strWalletFile)
{
...
CzMMCTracker::~CzMMCTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzMMCTracker::Init()
{
...
bool CzMMCTracker::Archive(CMintMeta& meta)
{
...
bool CzMMCTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzMMCTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzMMCTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzMMCTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzMMCTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzMMCTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzMMCTracker::GetSerialHashes()
{
...
CAmount CzMMCTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzMMCTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzMMCTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzMMCTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzMMCTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzMMCTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzMMCTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzMMCTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzMMCTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzMMCTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzMMCTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzMMCWallet* zMMCWallet)
{
    bool iszMMCWalletInitialized = (NULL != zMMCWallet);
    CMintMeta meta;
    ...
    if (! iszMMCWalletInitialized)
        zMMCWallet = new CzMMCWallet(strWalletFile);
    meta.isSeedCorrect = zMMCWallet->CheckSeed(dMint);
    if (! iszMMCWalletInitialized)
        delete zMMCWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzMMCTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzMMCTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzMMCTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzMMCTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzMMCTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzMMCTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzMMCWallet* zMMCWallet = new CzMMCWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zMMCWallet);
        }
        delete zMMCWallet;
        LogPrint("zero", "%s: added %d dzmmc from DB\n", __func__, listDeterministicDB.size());
    }
