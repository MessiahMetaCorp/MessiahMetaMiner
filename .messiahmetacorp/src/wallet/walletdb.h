...
public:
    ...
    bool WriteZMMCSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZMMCSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZMMCSeed_deprecated(uint256& seed);
    bool EraseZMMCSeed();
    bool EraseZMMCSeed_deprecated();

    bool WriteZMMCCount(const uint32_t& nCount);
    bool ReadZMMCCount(uint32_t& nCount);
