...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zmmcspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 MMC at the same time
...
enum ZerocoinSpendStatus {
    ZMMC_SPEND_OKAY = 0,                            // No error
    ZMMC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZMMC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZMMC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZMMC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZMMC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZMMC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZMMC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZMMC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZMMC_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZMMC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZMMC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZMMC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZMMC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZMMC_SPENT_USED_ZMMC = 14,                      // Coin has already been spend
    ZMMC_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZMMC_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZMMCOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzmmcAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzMMCTracker> zmmcTracker;
    ...
    void setZWallet(CzMMCWallet* zwallet)
    {
        zwalletMain = zwallet;
        zmmcTracker = std::unique_ptr<CzMMCTracker>(new CzMMCTracker(strWalletFile));
    }

    CzMMCWallet* getZWallet() { return zwalletMain; }
    ...
    /** zMMC reset */
    boost::signals2::signal<void()> NotifyzMMCReset;
